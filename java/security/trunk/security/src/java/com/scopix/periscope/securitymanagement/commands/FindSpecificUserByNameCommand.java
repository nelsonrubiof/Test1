 ...
 */
public class FindSpecificUserByNameCommand {

    Logger log = Logger.getLogger(FindSpecificUserByNameCommand.class);

    public PeriscopeUser execute(String name) throws ScopixException {
        log.debug("start");
        SecurityManagementHibernateDAO dao = SpringSupport.getInstance().
                findBeanByClassName(SecurityManagementHibernateDAO.class);

        PeriscopeUser periscopeUser = dao.findSpecificUserByName(name);
