...
public class SituationRequestDAOImpl implements SituationRequestDAO {
	...
	private static final String SAVE = "INSERT INTO situation_request(duration, frecuency, last_process, priorization, random_camera, "
			+ "situation_template_id, extraction_plan_id) VALUES (?, ?, ?, ?, ?, ?, ?);";
	...
	public Integer save(SituationRequest value) {
		...
		final Timestamp lastProcess = (value.getLastProcess() == null ? null : new Timestamp(value.getLastProcess().getTime()));
		...
		this.jdbcTemplate.update(new PreparedStatementCreator() {
			...
			public PreparedStatement createPreparedStatement(Connection cnctn) throws SQLException {
				...
				ps.setTimestamp(3, lastProcess);
				ps.setObject(4, priorization);
				ps.setObject(5, randomCamera);
				ps.setObject(6, situationTemplateId);
				ps.setObject(7, extractionPlanId);
				return ps;
			}
		}, keyHolder);
