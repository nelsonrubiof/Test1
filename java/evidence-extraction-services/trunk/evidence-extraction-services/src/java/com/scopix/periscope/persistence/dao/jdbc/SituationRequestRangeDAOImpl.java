...
public class SituationRequestRangeDAOImpl implements SituationRequestRangeDAO {
	...
	public Integer save(SituationRequestRange srr) {
		...
		final Timestamp endTime = (srr.getEndTime() == null ? null : new Timestamp(srr.getEndTime().getTime()));
		final Integer frequency = srr.getFrecuency();
		final Timestamp initialTime = (srr.getInitialTime() == null ? null : new Timestamp(srr.getInitialTime().getTime()));
		final SituationRequestRangeType rangeType = srr.getRangeType();
		...
		this.jdbcTemplate.update(new PreparedStatementCreator() {
			...
			public PreparedStatement createPreparedStatement(Connection cnctn) throws SQLException {
				...
				ps.setString(6, rangeType.getName());
