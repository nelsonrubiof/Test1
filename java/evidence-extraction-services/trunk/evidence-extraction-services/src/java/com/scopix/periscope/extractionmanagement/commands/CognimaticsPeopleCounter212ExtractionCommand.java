 ...
 *
 * CognimaticsPeopleCounter212ExtractionCommand.java
 *
 ...
 */
@SpringBean
public class CognimaticsPeopleCounter212ExtractionCommand implements
        ProviderAdaptor<CognimaticsPeopleCounter212ExtractionRequest, CognimaticsPeopleCounter212EvidenceProvider> {

    private Date date;
    private static Logger log = Logger.getLogger(CognimaticsPeopleCounter212ExtractionCommand.class);
    private SchedulerManager schedulerManager;

    public CognimaticsPeopleCounter212ExtractionCommand() {
    }

    public void execute(CognimaticsPeopleCounter212ExtractionRequest pcER, CognimaticsPeopleCounter212EvidenceProvider evProv,
    ...
    @Override
    public void prepareEvidence(CognimaticsPeopleCounter212ExtractionRequest evidenceRequest,
            CognimaticsPeopleCounter212EvidenceProvider evidenceProvider)
...
//            String url = protocol + "://" + serverIP + ":" + port + "/local/people-counter/.api?export-xml&date=all&res=15m";
            String url = protocol + "://" + serverIP + ":" + port + "/local/people-counter/.api?export-xml"
                    + "&date=" + range + "&res=15m";
            ...
            callPeopleCounting(url, user, pass, name, evidenceRequest);
        } catch (Exception e) {
    ...
    private void callPeopleCounting(String urlString, String user, String password, String fileName,
            CognimaticsPeopleCounter212ExtractionRequest evidenceRequest) throws ScopixException {
        ...
        try {
            ...
            if (response != null && response.getEntity() != null) {
                ...
                Map<String, Map<String, Integer>> data = this.readPeopleCountingFile(response.getEntity().getContent(), evidenceRequest);
    ...
    public Map<String, Map<String, Integer>> readPeopleCountingFile(InputStream is,
            CognimaticsPeopleCounter212ExtractionRequest evidenceRequest) throws ScopixException {
        ...
        for (CntSetXmlDefinition212 cntset : cpcxd.getCntset()) {
            ...
            for (CntGroupXmlDefinition212 cntgroup : cntset.getCntGroups()) {
                log.debug("endTime = " + cntgroup.getEndtime());
                ...
                if (existDiff) {
                    //double d = TimeZoneUtils.getDiffInHoursTimeZone(getSchedulerManager().getTimeZoneId());
                    double d = TimeZoneUtils.getDiffInHoursTimeZone(evidenceRequest.getExtractionPlan().getTimeZoneId());
                ...
                if (initCarga.getTime().before(cal.getTime())) {
                    ...
                    String dateKey = DateFormatUtils.format(cal.getTime(), "yyyy-MM-dd HH:mm:ss");
    ...
    private void configureDate(Calendar dia, CognimaticsPeopleCounter212ExtractionRequest evidenceRequest) throws ParseException {
