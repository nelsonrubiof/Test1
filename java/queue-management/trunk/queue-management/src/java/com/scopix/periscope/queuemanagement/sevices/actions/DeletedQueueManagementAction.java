 ...
 *
 * DeletedQueueManagementAction.java
 *
 ...
 */
@Validation
@Results({ @Result(name = "success", value = "/WEB-INF/jsp/queuemanagement/deleted/deletedList.jsp"),
		@Result(name = "filter", value = "/WEB-INF/jsp/queuemanagement/deleted/filters/filters.jsp"),
		@Result(name = "areafilter", value = "/WEB-INF/jsp/queuemanagement/deleted/filters/areaFilter.jsp"),
		@Result(name = "list", value = "/WEB-INF/jsp/queuemanagement/deleted/deletedListTable.jsp") })
@Namespace("/queuemanagement")
@ParentPackage(value = "default")
public class DeletedQueueManagementAction extends BaseAction implements SessionAware {

	private static final long serialVersionUID = 4655969869845504553L;

	Logger log = Logger.getLogger(DeletedQueueManagementAction.class);
	...
	public String list() throws ScopixException {
		...
		if (getFilters() == null) {
			...
			filters = (FilteringData) ServletActionContext.getRequest().getSession().getAttribute("DELETED-FILTERS");
		}
		if (filters != null) {
			if (getFilters().getDate() == null) {
				...
				if (queueId == null || queueId < 0) {
					...
					getFilters().setStatus(EvaluationState.DELETED.name());
					filters.setQueueNameId(queueId);
					ServletActionContext.getRequest().getSession().setAttribute("DELETED-FILTERS", filters);
	...
	public String readyList() {
		SpringSupport.getInstance().findBeanByClassName(QueueManager.class)
				.setChecked(getPendingEvaluationDTOs(), getAllSelected("deletedChecks"), true);
	...
	public String showAll() {
		setPageDeleted((pageDeleted == null || !pageDeleted.equals("30")) ? "30" : "" + getPendingEvaluationDTOs().size());
	...
	public String recover() throws ScopixException {
		List<Integer> checked = getAllSelected("deletedChecks");
		...
		if (checked.size() == 0) {
			...
			if (queueId == null || queueId < 0) {
				...
				QueueManagementWebServices webService = SpringSupport.getInstance()
						.findBeanByClassName(QueueManagementWebServiceClient.class).getWebService();
				long sessionId = session.get("sessionId") != null ? (Long) session.get("sessionId") : 0L;
				webService.backToQueue(checked, filters, getStartPosition(), EvaluationState.DELETED, sessionId);
				this.list();
				this.removeCheckedValues("deletedChecks");
			}
		}
		return LIST;
	}
	...
	public void setPageDeleted(String pageDeleted) {
