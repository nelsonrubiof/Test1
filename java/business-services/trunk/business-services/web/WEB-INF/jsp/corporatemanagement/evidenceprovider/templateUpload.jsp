...
<s:debug /> <hr>
        ...
        <script type="text/javascript">
            ...
            function showPreview(url){
                var fileBGClientPath = document.getElementById('fileBGClientPath').value;
                var fileClientPath = document.getElementById('fileClientPath').value;
                url += '&fileClientPath='+fileClientPath+'&fileBGClientPath='+fileBGClientPath;
                document.getElementById('uploadForm').action= url;
                document.getElementById('uploadForm').submit();
            }
...
--%>
    ...
    <body>
        <s:url id="previewUrl" namespace="/corporatemanagement" action="evidenceProvider!preview">
            <s:param name="evidenceProviderId" value="evidenceProvider.id"/>
        </s:url>
        <s:url id="uploadFileUrl" namespace="/corporatemanagement" action="evidenceProvider!uploadFile"/>
        <h5 align="center">
        ...
        <s:form  id="uploadForm" name="uploadForm" cssStyle="border:1px;width:100%" enctype="multipart/form-data" method="POST">
            ...
            <s:select key="label.situationTemplate" name="sitautionTemplateId" list="situationTemplates"
              listKey="id"
              listValue="name"
              headerKey="-1"
              required="true" />
            <s:file name="previewUpload" key="label.uploadImageBack" id="uploadBack" value="%{fileBGClientPath}" id="fileBGClientPath"/>
                ...
                <td>
                    <table align="right" style="text-align:right;">
                        <s:a onclick="showPreview('%{previewUrl}');return false;" showLoadingText="false" indicator="waitDiv" cssClass="button">
                            <span><s:text name="label.preview"/></span>
                        </s:a>
                    </table>
                </td>
            </tr>
        ...
        <s:if test="#attr.PREVIEW != null">
            <s:url id="showBackUrl" action="evidenceProvider" namespace="/corporatemanagement" method="getBackgroundImage"/>
            <s:url id="showFrontUrl" action="evidenceProvider" namespace="/corporatemanagement" method="getImage"/>
            <img src="${showBackUrl}" style="position:absolute;left:50px;top:150px;" height="480px" width="640px">
            <img src="${showFrontUrl}" style="position:absolute;left:50px;top:150px;" height="480px" width="640px">
