 ...
 */
public class RejectEvidenceFinishedCommand {

    private Logger log = Logger.getLogger(RejectEvidenceFinishedCommand.class);
    private QualityControlHibernateDAO dao;

    public void execute(List<Integer> observedMetricIds, String comments) throws ScopixException {
        log.debug("start");
        getDao().rejectsEvaluations(observedMetricIds, comments);
        log.debug("end");
    }
