/*
 *  
 *  Copyright (C) 2007, SCOPIX. All rights reserved.
 * 
 *  This software and its documentation contains proprietary information and can
 *  only be used under a license agreement containing restrictions on its use and
 *  disclosure. It is protected by copyright, patent and other intellectual and
 *  industrial property laws. Copy, reverse engineering, disassembly or
 *  decompilation of all or part of it, except to the extent required to obtain
 *  interoperability with other independently created software as specified by a
 *  license agreement, is prohibited.
 * 
 * 
 *  MetricEvaluatorNumberInput.java
 * 
 *  Created on 15-06-2012, 12:12:01 PM
 * 
 */
package com.scopix.periscope.evaluationmanagement.evaluators;

import com.scopix.periscope.evaluationmanagement.EvidenceEvaluation;
import com.scopix.periscope.evaluationmanagement.MetricEvaluation;
import com.scopix.periscope.evaluationmanagement.ObservedMetric;
import com.scopix.periscope.periscopefoundation.exception.ScopixException;
import com.scopix.periscope.periscopefoundation.util.config.HibernateSupport;
import org.apache.log4j.Logger;

/**
 *
 * @author Gustavo Alvarez
 */
public class MetricEvaluatorNumberInput implements MetricEvaluator {

    private Logger log = Logger.getLogger(MetricEvaluatorNumberInput.class);

    @Override
    public void evaluate(ObservedMetric observedMetric) throws ScopixException {
        log.info("start");
        MetricEvaluation metricEvaluation = new MetricEvaluation();
        metricEvaluation.setObservedMetric(observedMetric);
        int count = 0;
        boolean saveResult = false;
        log.debug("observedMetric.getEvidenceEvaluations().size(): " + observedMetric.getEvidenceEvaluations().size());
        for (EvidenceEvaluation ee : observedMetric.getEvidenceEvaluations()) {
            if (!ee.isRejected() && ee.getEvidenceResult() != null) {
                //debiese existir solo una no rechazada
                saveResult = true;
                count = ee.getEvidenceResult();
            }
        }
        log.debug("saveResult: " + saveResult + ", count: " + count);
        if (saveResult) {
            metricEvaluation.setMetricEvaluationResult(count);
            HibernateSupport.getInstance().findGenericDAO().save(metricEvaluation);
        }
        log.info("end");
    }
}
