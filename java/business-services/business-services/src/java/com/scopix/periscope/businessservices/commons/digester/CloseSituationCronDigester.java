 ...
 * @author Admin
 */
@SpringBean(rootClass = CloseSituationCronDigester.class)
public class CloseSituationCronDigester implements InitializingBean {

    private static final Logger log = Logger.getLogger(CloseSituationCronDigester.class);
    ...
    public PropertiesConfiguration getConfiguration() {
        if (configuration == null) {
            try {
                configuration = new PropertiesConfiguration("close_situation_cron.properties");
    ...
    private void generateCronJob() {
        log.info("start");
        String[] execution = getConfiguration().getStringArray("close_situation_cron.execution");
        for (String hourMin : execution) {
            String[] data = StringUtils.split(hourMin, ":");
            try {
                JobDetail job = new JobDetail("CloseSituationCronDigester_" + hourMin, "CloseSituationCronDigester",
                        CloseSituationCronJob.class);
                Trigger trigger = TriggerUtils.makeDailyTrigger(Integer.parseInt(data[0]), Integer.parseInt(data[1]));
                trigger.setName("CloseSituationCronDigesterTrigger_" + hourMin);
