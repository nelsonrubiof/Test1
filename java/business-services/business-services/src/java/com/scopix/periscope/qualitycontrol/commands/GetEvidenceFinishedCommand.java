 ...
 */
public class GetEvidenceFinishedCommand {

    private static Logger log = Logger.getLogger(GetEvidenceFinishedCommand.class);
    private QualityControlHibernateDAO dao;

    public List<ObservedMetricResultDTO> execute(FilteringData filters, Corporate corporate) throws ScopixException {
        log.debug("start");
        

        List<ObservedMetricResultDTO> evidenceFinishedDTOs = null;
        if (corporate.getUniqueCorporateId() == 6) { // CASO LOWES
            evidenceFinishedDTOs = getDao().getEvidenceFinishedListLowes(filters);
        } else {
            evidenceFinishedDTOs = getDao().getEvidenceFinishedList(filters);
        }
