 ...
 * @author Admin
 */
public class GetEvidenceForStoreDaySituationRealCommand {

    private static final Logger log = Logger.getLogger(GetEvidenceForStoreDaySituationRealCommand.class);
    ...
    public List<StorePlan> execute(Date date) {
        log.info("start " + date);
        List<StorePlan> ret = getDao().getEvidenceForStoreDaySituationReal(date);
