 ...
 * @author Nelson
 */
public class DSRThread extends Thread {

    private static Logger log = Logger.getLogger(DSRThread.class);
    ...
    public void run() {
        log.info("start");
        try {
            DSRExternalApplication dsr = SpringSupport.getInstance().findBeanByClassName(DSRExternalApplication.class);
            dsr.sendData(getObservedSituationId());
        } catch (ScopixException e) {
            log.warn("no se pudo enviar data de observed situation " + e);
        }
        log.info("end");
