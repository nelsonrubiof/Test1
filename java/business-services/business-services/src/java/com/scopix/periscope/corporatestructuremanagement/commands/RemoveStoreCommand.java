 ...
 *
 * Created on 07-05-2008, 03:11:29 PM
 *
 */
...
public class RemoveStoreCommand {
    ...
    public void execute(Integer id) throws ScopixException {
        ...
        try {
            Store aux = dao.get(id, Store.class);
            if (aux.getEvidenceExtractionServicesServer() != null && aux.getEvidenceExtractionServicesServer().getId() != null &&
                    aux.getEvidenceExtractionServicesServer().getId() > 0) {
                EvidenceExtractionServicesServer eess = dao.get(aux.getEvidenceExtractionServicesServer().getId(),
                        EvidenceExtractionServicesServer.class);
                //eess.setStore(null);
                //eess.delStore(storeId);
            }
            dao.remove(id, Store.class);
